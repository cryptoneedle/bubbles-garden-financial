...
if __name__ == '__main__':
    ...
    with engine.begin() as conn:
        conn.execute(text("DROP TABLE IF EXISTS baostock." + DividendData.TABLE_NAME))
    DividendData.METADATA.create_all(engine, checkfirst=True)

    stocks = get_all_stocks()
    for index, row in tqdm(stocks.iterrows(), total=len(stocks), desc="处理股票数据"):
        code = row['证券代码']
        for year in range(1990, int(next_year)):
